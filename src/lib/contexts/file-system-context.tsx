...
export function FileSystemProvider({
  ...
  const handleToolCall = useCallback(
          ...
          case "create":
            if (path && file_text !== undefined) {
              if (fileSystem.exists(path)) {
                // File already exists: overwrite and refresh
                updateFile(path, file_text);
              } else {
                fileSystem.createFileWithParents(path, file_text);
                triggerRefresh();
              }
            }
            break;
    ...
    [fileSystem, createFile, updateFile, deleteFile, renameFile, triggerRefresh]
  );
